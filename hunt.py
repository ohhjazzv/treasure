...
get_stats() -> lifetime stats + unlocked achievements
...
DIFFICULTy
    ...
    density before your new_game(). Dosent touch anything else abt level scaling.
"""
...
def _level_config(level):
    ...
    traps = int(traps * preset["trap_multiplier"])
    shovels = max(STARTING_SHOVELS - (level - 1) // 2, MIN_SHOVELS)
    shovels += preset["shovel_bonus"]
    shovels = max(shovels, 1)
...
def _weighted_treasure(level):
    """In this u will have to pick a teasure type, weighted so the golden idols
    get more common the deeper you are, with small bumps per level, capped so it
    cant be abused and dominated muhehehehe"""
    available = [t for t in TREASURES_TYPES if level >= t.get("min_level", 1)]
    weights = []
    for t in available:
        w = t["weight"]
...
def _weighted_trap(level):
    """PICk a trap type, weighted so the instant expolsive mine stays rare but
    creeps up as lvl goes on"""
    weights = []
    for t in TRAP_TYPES:
        w = t["weight"]
        if t.get("instant"):
...
def _place_items(size, num_treasures, num_traps, level):
    """Scatter treasure and traps across a size x size grid with no two landing
    on the same square. Returns (hidden_grid, treasure_positions)."""
    num_treasures = min(num_treasures, size * size)
    num_traps = max(0, min(num_traps, size * size - num_treasures))
    ...
    for (r, c) in treasure_spots:
        hidden[r][c] = {"type": "treasure", "treasure": _weighted_treasure(level), "trap": None}
...
def _distance(r1, c1, r2, c2):
    """Taxicab diistance - squares across plus squares down. no diagonals"""
    return abs(r1 - r2) + abs(c1 - c2)


def _hint_for(row, col):
    """Distance-based hint to the nearest still-buried treausre. Returns None if
    smhow there are no treasures left (shouldnt happen mid lvl, since finding
    the last one triggers a level before another dig)"""
...
def _check_achievements(event, **kwargs):
    """Small achievements tracker. CHeap enough to just compute a set and write
    it back each time smthing noteworthy happens."""
...
def _build_level(level):
    """This block sets _state/_hidden/_treasure_positions for the given level,
    keeping whatever coins/shovel bonus should carry over (caller's job to have
    already adjusted shovels before calling this for lvl > 1)"""
    ...
    _hidden, _treasure_positions = _place_items(
        config["size"], config["treasures"], config["traps"], level
    )
    _check_achievements("level_reached", level=level)


def _advance_level():
    """Called the moment the last treasure on a level is dug up. Rolls leftover
    shovels (capped) into the next level and makes it harder"""
...
def _update_leaderboard():
    """Drop this run's result into the leaderboard, keep it sorted by coins
    descending and trim it back down to top N"""
...
def _end_game():
    """Called when shovels hit 0 with treasures still unfound. Locks the run,
    updates lifetime stats/high score, and saves to disk."""
    _state["over"] = True
...
def new_game():
    """Starts a brand new run at lvl 1, wipes the current grid/coins/shovels.
    Does not touch lifetime stats/high score - those only update when a run
    actually ends"""
    global _state
...
def dig(row, col):
    ...
    ok      - False if the move couldn't happen at all (out of bounds, no
              shovels, game already over). True otherwise, even for a trap hit -
              the dig itself succeeded, it just went badly.
    message - ready-to-print line describing what happened.
    result  - "treasure" / "trap" / "empty" / "already_dug", or None when ok is
              False.
    hint    - "burning" / "hot" / "warm" / "cold" on an empty result, otherwise
              None.
    """
    if not _state:
    ...
    ## already dug portion - free look, no shovel cost
    if cell != "sand":
    ...
    if hidden_cell["type"] == "treasure":
        ...
        if (row, col) in _treasure_positions:
...
def get_state():
    """Snapshot of the current run. Grid is copied so the caller cant
    accidentally mutate internal state by poking at the returned list"""
    if not _state:
        return {
            "grid": [], "shovels": 0, "coins": 0, "level": 0,
            "treasures_left": 0, "over": True, "grid_size": 0, "streak": 0,
        }
...
def set_difficulty(name):
    """Switch difficulty preset ("easy" / "normal" / "hard"). Takes effect on the
    next new_game() - doesn't touch a run already in progress.
    Returns True if it was a real preset name, False otherwise"""
    global _difficulty
...
def get_difficulty():
    """Currently active difficulty preset name."""
    return _difficulty


def get_leaderboard():
    """Top runs ever, sorted best-first, each as {"coins": int, "level": int}.
    Capped at _MAX_LEADERBOARD_ENTRIES entries."""
    return [dict(entry) for entry in _stats.get("leaderboard", [])]


def reset_stats():
    """Wipe all lifetime stats/achievements/leaderboard/high score back to
    defaults and save that over whatever was on disk. Doesn't touch the current
    in-progress run. Useful for a "reset high score" menu option"""
    global _stats
...
def get_legend():
    """Static reference info abt treasures/traps/hints, handy for a 'how to
    play' screen. Doesn't reveal anything about the current grid, just the
    general rules, same for every run."""
    return {
        "treasures": [
        ...
        "hints": [{"max_distance": d, "name": name} for d, name in HINT_TIERS]
                 + [{"max_distance": None, "name": "cold"}],
...
def buy_shovel():
    """Spends SHOVEL_COST_IN_COINS coins for one extra shovel mid-run.
    Returns {ok, message}. Purely optional - main.py doesnt have to expose this
    at all"""
